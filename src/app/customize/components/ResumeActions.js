...
import { setMyCVOptions } from "@/store/features/cvs/cvsThunks";
...
export default function ResumeActions() {
  const { myCVOptions = {} } = useAppSelector((state) => state.cvs);
  const dispatch = useAppDispatch();

  const { register, watch, setValue } = useForm({
    defaultValues: Object.entries(myCVOptions).reduce(
  ...
  const handleCheckboxChange = async (sectionKey, fieldKey) => {
    setValue(
      `${sectionKey}-${fieldKey}`,
    ...
    await dispatch(
      setMyCVOptions({
        sectionKey,
        fieldKey: fieldKey,
        isSelected: !formValues[`${sectionKey}-${fieldKey}`],
      })
    );
  };
    ...
    <div className="p-6 py-10 space-y-6">
      ...
      <div>
        ...
        <div className="space-y-6">
          {Object.entries(myCVOptions).map(([sectionKey, section]) => (
